...
static inline void DisableWriteGatherPipe(void) { PPCMthid2(PPCMfhid2() & ~HID2_WPE); }

static inline u32 __OSGetHollywoodRev(void) { return *(u32*)OSPhysicalToCached(OS_PHYS_HOLLYWOOD_REV); }

void __OSGetIOSRev(OSIOSRev* rev) {
    u32 version = *(u32*)OSPhysicalToUncached(OS_PHYS_IOS_VERSION); // 3140
    u32 builddate = *(u32*)OSPhysicalToUncached(OS_PHYS_IOS_BUILD_DATE); // 3144
...
static inline void MemClear(void* mem, u32 size) {
...
static void ClearArena(void) {
    ...
    else if (__OSRebootParams.regionStart == NULL || !OSIsMEM1Region(__OSRebootParams.regionStart)) {
        MemClear(OSGetArenaLo(), (u32)OSGetArenaHi() - (u32)OSGetArenaLo());
    }
...
static void ClearMEM2Arena(void) {
    ...
    else if (__OSRebootParams.regionStart == NULL || !OSIsMEM2Region(__OSRebootParams.regionStart)) {
        MemClear(OSGetMEM2ArenaLo(), (u32)OSGetMEM2ArenaHi() - (u32)OSGetMEM2ArenaLo());
    }
    // Region begins after arena begins
...
static inline void CheckTargets(void) {
...
void OSInit(void) {
    ...
    if (!AreWeInitialized) {
        ...
        PPCMtpmc3(0);
        PPCMtpmc4(0);
        PPCMthid4(HID4_H4A | HID4_SBE | HID4_PS1_CTL | (1 << 23) | HID4_L2_CCFI);
        ...
        if (mem2lo != NULL) {
            ...
            else if (mem2lo >= (void*)0x90000000 && mem2lo < (void*)0x90000800) {
...
static void OSExceptionInit(void) {
    ...
    for (i = 0; i < OS_EXC_MAX; i++) {
        if (BI2DebugFlag != NULL && *BI2DebugFlag >= 2 && __DBIsExceptionMarked(i)) {
            ...
            *pInst = inst | i;
            ...
            if (__DBIsExceptionMarked(i)) {
                ...
                memcpy(__DBVECTOR, __OSDBJUMPSTART, (u32)__OSDBJUMPEND - (u32)__OSDBJUMPSTART);
            } else {
                code = (u32*)__DBVECTOR;
                for (j = 0; j < (u32)__OSDBJUMPEND - (u32)__OSDBJUMPSTART; j += sizeof(u32), code++) {
                    // Write nop
                    *code = 0x60000000;
                }
            }
    ...
    OSExceptionTable = (OSExceptionHandler*)OSPhysicalToCached(OS_PHYS_EXCEPTION_TABLE);
    for (i = 0; i < OS_EXC_MAX; i++) {
...
OSExceptionHandler __OSGetExceptionHandler(u8 type) { return OSExceptionTable[type]; }
...
const char* OSGetAppGamename(void) {
    int i;
    const char* name = (const char*)OSPhysicalToCached(OS_PHYS_CURRENT_APP_NAME);
