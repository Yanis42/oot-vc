...
static void __GXXfVtxSpecs(void) {
    ...
    txc = 32 -
          __cntlzw(GX_BITGET(gx->vcdHi, GX_CP_VCD_HI_TEX7COORD_ST,
                             sizeof(u16) * 8)) +
          1;
...
void GXGetVtxDesc(GXAttr name, GXAttrType* type) {
    ...
    case GX_VA_NRM:
        result =
            gx->hasNrms ? GX_CP_GET_VCD_LO_NORMAL(gx->vcdLo) : GX_NONE;
        break;
    case GX_VA_NBT:
        result =
            gx->hasBiNrms ? GX_CP_GET_VCD_LO_NORMAL(gx->vcdLo) : GX_NONE;
        break;
    case GX_VA_CLR0:
