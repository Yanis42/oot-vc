...
typedef enum {
    ...
    IPC_HEAP_CHUNK_FOR_ALIGN = 0xBABE0002
};
...
void* __iosAlloc(s32 handle, u32 size, u32 align) {
    ...
    // To find the real chunk, you take the FOR_ALIGN one and jump back one.
    block = (u8*)best + bestUnAlign + sizeof(IPCHeapChunk);
    if (bestUnAlign != 0) {
        IPCHeapChunk* aligned = (IPCHeapChunk*)(block - sizeof(IPCHeapChunk));
        aligned->magic = IPC_HEAP_CHUNK_FOR_ALIGN;
        aligned->prev = best;
    }
...
s32 iosFree(s32 handle, void* block) {
    ...
    if (chunk->magic == IPC_HEAP_CHUNK_FOR_ALIGN) {
        chunk = chunk->prev;
    }
