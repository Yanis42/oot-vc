...
static s32 nandCreate(const char* path, u8 perm, u8 attr, NANDCommandBlock* block, bool async, bool priv) {
    char absPath[64];
...
static s32 nandMove(const char* from, const char* to, NANDCommandBlock* block, bool async, bool priv) {
    char absPathFrom[64];
    char absPathTo[64];
    ...
    if (!priv && (nandIsPrivatePath(absPathFrom) || nandIsPrivatePath(absPathTo))) {
        return IPC_RESULT_ACCESS;
    }
