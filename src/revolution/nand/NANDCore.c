...
static s32 _ES_GetDataDir(s32* fd, u64 tid, char* dirOut) NO_INLINE;
...
static inline bool nandIsRelativePath(const char* path) { return *path == '/' ? false : true; }
...
static s32 nandGetType(const char* path, u8* type, NANDCommandBlock* block, bool async, bool priv) {
    ...
    if (async) {
        ...
        block->type = type;
        return ISFS_ReadDirAsync(block->path, NULL, &block->dirFileCount, nandGetTypeCallback, block);
    } else {
        MEMCLR(&absPath);
...
static inline s32 _ES_CloseLib(s32* fd) {
