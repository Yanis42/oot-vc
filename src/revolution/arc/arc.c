...
static u32 entryToPath(ARCHandle* handle, u32 entrynum, char* string, u32 maxlen);
...
static bool ARCConvertEntrynumToPath(ARCHandle* handle, s32 entrynum, char* string, u32 maxlen) {
...
bool ARCFastOpen(ARCHandle* handle, s32 entrynum, ARCFileInfo* info) {
    ...
    if (entrynum < 0 || entrynum >= handle->count || ARCNodeIsFolder(nodes[entrynum])) {
        return false;
    }
...
s32 ARCConvertPathToEntrynum(ARCHandle* handle, const char* path) {
    ...
    compare:
        ...
        while (it < nodes[anchor].folder.sibling_next) {
            while (true) {
                if (ARCNodeIsFolder(nodes[it]) || name_delimited_by_slash != true) {
                    char* name_of_it = ((char*)handle->strings) + ARCNodeGetName(nodes[it]);
...
u32 ARCEntrynumIsDir(ARCHandle* handle, u32 entrynum) {
    return ARCNodeIsFolder(handle->nodes[entrynum]) ? true : false;
}
...
void* ARCGetStartAddrInMem(ARCFileInfo* info) { return (u8*)info->handle->header + info->offset; }
...
bool ARCReadDir(ARCDir* dir, ARCEntry* entry) {
    ...
    while (true) {
        ...
        entry->type = ARCNodeIsFolder(nodes[it]) ? ARC_ENTRY_FOLDER : ARC_ENTRY_FILE;
