...
u32 MEMGetAllocatableSizeForExpHeapEx(MEMiHeapHead* heap, s32 Align) {
    ...
    for (it = exp->freeMBlocks.head; it != NULL; it = it->next) {
        ...
        if (AlignStart < end) {
            ...
            // padding
            if (biggestSize < size || (biggestSize == size && smallestPadding > padding)) {
                biggestSize = size;
...
u32 MEMAdjustExpHeap(MEMiHeapHead* heap) {
    ...
    // Last free block
    else if (AddU32ToPtr(mblock, mblock->size + sizeof(MEMiExpHeapMBlock)) != heap->end) {
        size = 0;
    }
    // Shrink heap by the last free block
    else {
        RemoveMBlock_(&exp->freeMBlocks, mblock);
        heap->end = SubU32ToPtr(heap->end, mblock->size + sizeof(MEMiExpHeapMBlock));
        // New adjusted size
